package com.caibang.www.config.filter;
...
//    }
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        log.info("myFilter request url :[{}]", requestURL);
        if (requestURL.toString().contains("localhost:2221")) {
            if (session.getAttribute("sid") != null) {
                try {
                    String sessionId = (String) redisUtils.get(session.getAttribute("sid").toString());
                    /**
                     * session+redis
                     * session共享   唯一登录
                     */
    ...
    @Override
    public void destroy() {
        log.info("myFilter was destroyed");
    }
}
